...
local BAR_CONFIG = {
    {
        xb = 52, yb = 225, name = 'upspeedf', arg = 'enp7s0', max = 20000, nb_blocks = 50,
        cap = CAIRO_LINE_CAP_SQUARE, w = 9, h = 4, space = 1,
        warning = 75, alarm = 90,
        led_effect = true, led_alpha = 0.8, rotation = 90
    },
    {
        xb = 52, yb = 242, name = 'downspeedf', arg = 'enp7s0', max = 100000, nb_blocks = 50,
        cap = CAIRO_LINE_CAP_SQUARE, w = 9, h = 4, space = 1,
        warning = 75, alarm = 90,
        led_effect = true, led_alpha = 0.8, rotation = 90
    }
}
local BG_STRIPE = {start_x = 10, start_y = 268, pair_height = 31, total_width = 290, max_pairs = 25}
...
for _, params in ipairs(BAR_CONFIG) do
    ...
    if conn_cache.last_update ~= upd then
        ...
        -- NETWORK CONNECTED – temporarily disabled for debugging
        local function is_network_connected()
        return true  -- always draw bars, even without IP
                                                        ...
                                                        for _, entry in ipairs(last_display_entries) do
                                                            ...
                                                            if entry.direction == "in" then
                                                                ...
                                                                .. "${voffset -1}${goto 4}${template4}└ ${color3}${template3}" .. info.host .. "\n"
                                                                    ...
                                                                    if not last_display_entries then return end
